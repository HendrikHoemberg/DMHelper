...
| 6. Map editor | 2026-08-02 | Tasks 38–41; `./mvnw -P gates test` green. Commits `7792556a` (four-region split), `3d8eb584`/`c593a88a` (contextual inspector, kept in sync with keyboard tool switches), `196cd77b`/`b3432547` (canvas legibility, quiet save state), `231c6701` (stage close). `MapEditorLayoutContractTest` and `MapEditorRenderGateTest` created as specified. The rail ships 11 tools: the product has never had the corridor tool spec 13.2 lists, and inventing one would be a functional change. Screenshots: `target/ui-redesign/map-editor/`. |
| 7. Session cockpit | 2026-08-02 | Tasks 42–46; `./mvnw -P gates test` green. Commits `1a7fe06c`/`055d59c2` (command bar on the eight approved slots), `274a3640`/`76c7e57a` (four built-in presets rebuilt around one primary task), `d3442cd2` (module ranking and runtime state), `1f0c12de`/`7997cd48` (container queries and laptop fit), `a8538325` (stage close, DM manual realigned). `CockpitSurfaceContractTest` and `CockpitLaptopFitGateTest` created as specified. Exploration's primary ratio was raised to 0.64 and Combat's bottom utility lowered to 0.16 against the plan's proposals; both stay inside `CockpitLayoutValidator`. Screenshots: `target/ui-redesign/cockpit/`. |
| 8. Consistency and release gate | 2026-08-03 | `./mvnw clean verify -P gates` green (2638 tests, 6 skipped). Tasks 47–53: presentation surface clean-canvas, admin/About/error pages, legacy compatibility-layer removal, form sweep, viewport/zoom/keyboard matrix, visual review matrix, and the whole-product release gate (see Release evidence below). Release-gate remediation on the browser matrix: SVG `<use>` elements were no longer counted as focusable, index card titles moved to `h2` under the page `h1` (spec 17 outline), the map editor gained a screen-reader-only `h1`, the cockpit command bar compacts the campaign identity below 1120px (1366×768@125% was clipping the "More" control by 4px), the combat preset's bottom utility ratio and the quick-notes minimum were both reduced so the strip stays ≤160px at 1000px, and the duplicate Ctrl+R handler in `topbar.js` (which double-toggled the dice roller against `keyboard.js`) was removed. Screenshots: target/ui-redesign/matrix/. |

## Release evidence

Task 53. Evidence for each of the twelve acceptance criteria in spec section 21. Criteria 4,
5, and 8 are human judgement calls made against `target/ui-redesign/matrix/`; the reviewer's
conclusion is recorded rather than asserted in code.

| # | Acceptance criterion | Evidence |
|---|---|---|
| 1 | Every shipped DM-facing page is assigned to and rendered with an approved archetype | `RedesignCoverageContractTest#everyShippedPageIsAssignedAnApprovedArchetype`, `ShellCompositionContractTest#everyStandardPageHasExactlyOneShellOneHeadingAndAtMostOnePrimaryAction`, `ShellRenderGateTest` |
| 2 | No shipped page uses the former brown-on-brown surface system | `RedesignCoverageContractTest#noShippedPageUsesTheFormerBrownSurfaceSystem` |
| 3 | Gold is absent from generic card borders and ordinary decorative separators | `RedesignCoverageContractTest#goldIsAbsentFromGenericCardBordersAndDecorativeSeparators`, `GoldAccentContractTest` |
| 4 | Primary action and current state are identifiable on every reviewed page | Human judgement vs `target/ui-redesign/matrix/populated/` and `target/ui-redesign/matrix/overlays/` — **reviewer conclusion: PASS** |
| 5 | Campaign, preparation, reference, editor, and runtime surfaces feel related but appropriately specialized | Human judgement vs `target/ui-redesign/matrix/populated/` (narrative, operational, reference, editor, session) — **reviewer conclusion: PASS** |
| 6 | Every built-in cockpit preset is useful without layout editing | `CockpitLaptopFitGateTest#everyBuiltInPresetFitsWithoutClippingOrDocumentScroll`, `CockpitBuiltInPresetCatalogTest`, `CockpitLayoutPresetServiceTest` |
| 7 | The cockpit and map editor remain fully operable at 1280×720 | `CockpitLaptopFitGateTest`, `MapEditorRenderGateTest#nothingEssentialClipsAtTheMinimumViewport`, `CoreSessionLoopSmokeTest#cockpitViewportGeometryHoldsAcrossSizesAndZoom` |
| 8 | Standard pages use ultrawide space intentionally and do not strand narrow content in unrestricted empty canvases | Human judgement vs `target/ui-redesign/matrix/ultrawide/` (2560×1440, 37 standard pages), cross-checked against `populated/` at 1440×900 — **reviewer conclusion: PASS**. The matrix previously held no capture above 1440px, so this criterion was not judgeable from its own evidence; `VisualReviewMatrixGateTest#captureTheUltrawideReviewSet` now produces it. |
| 9 | All asynchronous components have explicit loading, empty, error, and degraded behavior | `AsyncStateContractTest`, `InteractionFailureContractTest`, `VisualReviewMatrixGateTest#captureEmptyStatesFromTheUnseededCampaign` and `#captureDegradedAndErrorStates` (`matrix/degraded/`), `OverlayBehaviorGateTest`, `PresentationSurfaceTest` |
| 10 | Accessibility, zoom, viewport, keyboard, browser-health, and functional regression gates pass | `ViewportAccessibilityGateTest`, `ViewportMatrixGateTest`, `KeyboardOperationGateTest`, `ShellRenderGateTest`, `CockpitLaptopFitGateTest`, `ReleaseRehearsalTest`, `CoreSessionLoopSmokeTest` (failures collected via `BrowserFailureCollector`) |
| 11 | Legacy palette and component compatibility rules are removed or confined to a documented, temporary allowlist with a removal task | `RedesignCoverageContractTest#everyDuplicateShellImplementationIsGone`, `LegacyVisualAliasRemovalTest`, Task 49 (legacy compatibility layer removed) |
| 12 | The visual review matrix has been captured and approved against the feature-complete synthetic campaign | `VisualReviewMatrixGateTest`, screenshots under `target/ui-redesign/matrix/`: `populated/` (39 surfaces), `ultrawide/` (37 at 2560×1440), `empty/` (13), `degraded/` (error page, presentation unavailable), `overlays/` (palette, dice, toast, confirmation dialog, presentation). Live combat and initiative are reviewed from `target/ui-redesign/cockpit/builtin-combat-active.png`; the product has no standalone initiative page. |

The full release gate (`./mvnw clean verify -P gates`, 2638 tests) passes with no skipped
browser gates; the six skipped tests are pre-existing documented `@Disabled` items. `docs/ui-polish-spec.md` is retired.
